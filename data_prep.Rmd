...
traits$gen_sp[33] <- "Vincetoxicum hirundinaria" #gives error ¿?
#FIX!
cleanSpecies(traits$gen_sp[33]) # ¿?

clean_sp <- data.frame(species = NA, matched_name2 = NA, synonym_names = NA, final_name = NA)
for(i in c(100,200,300,400,500)){
  temp <- cleanSpecies(traits$gen_sp[(i-99):i]) #maybe can be repeated with other database. now db == Itis
  clean_sp <- rbind(clean_sp, temp)
}
clean_sp <- clean_sp[-1,]
...
lin2 <- merge(traits, clean_trait_sp, all.x = TRUE)
dim(lin)
dim(lin2) #check!
...
#clean species with taxize
clean_us_sp <- cleanSpecies(trails2$gen_sp) #maybe can be repeated with other database. now db == Itis
#save output as is looong to test.
write.csv(clean_us_sp, "data/clean_us_sp.csv")
#load it when needed
clean_us_sp <- read.csv("data/clean_us_sp.csv", h = T)
